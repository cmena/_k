...
def keytab():
    # turn KEY_LEFTCONTROL to HYPER in emacs:
    #
    ...
    return {
        evdev.ecodes.KEY_LEFTCTRL   : evdev.ecodes.KEY_F13,
...
# todo: do a sketch of an xcape impl with timed events
...
class backslash(key):
    ...
    #  keyup, otherwise act like a meta key. sys.exit() if q is pressed while
    #  in use as a meta modifier.
    def __init__(self):
...
# todo: add a sample noting that caps lock can be used a control if used solo
class leftctrl(key):
    # when pressed solo go to the i3 container to the left (or right since i
    #  always run two vertical tab containers) otherwise emit KEY_LEFTCTRL.
    #  this will be mapped to KEY_CAPSLOCK.
    def __init__(self):
        super().__init__(passthrough=False)
    def keysolo(self):
...
class volkey(key):
    ...
    #  ismediawin(). currently the hold events are let through. seems ok ...
